...
const adminAuctionsRouteApi = getRouteApi('/admin/auctions')

/** Same as browse, plus ended — and no default exclusion of ended auctions. */
...
export function AdminAuctionsPage() {
  ...
  const navigate = useNavigate({ from: '/admin/auctions' })
  const searchState = adminAuctionsRouteApi.useSearch()
    ...
    ;(async () => {
      try {
        const res = await listAuctions({
          ...
          // Unlike public browse, do not exclude ended by default — show all sellers' auctions.
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold !text-gray-900 dark:!text-white">Admin — Auctions</h1>
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Search and filter every auction on the platform (all sellers). Open a card to view details.
        </p>
      </div>
